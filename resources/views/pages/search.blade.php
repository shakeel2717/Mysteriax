        ...
        <div class="row">
            <div class="col-12 col-sm-12 col-lg-8 col-md-7 second p-0">
                <div class="d-flex neutral-bg fixed-top-m px-3 py-3 feed-mobile-search">
                    <span
                        class="h-pill h-pill-primary rounded search-back-button d-flex justify-content-center align-items-center"
                    ...
                    @if ($activeFilter == 'people')
                        <span
                            class="h-pill h-pill-primary rounded search-back-button d-flex justify-content-center align-items-center"
                            data-toggle="collapse" href="#colappsableFilters" role="button" aria-expanded="false"
                            aria-controls="colappsableFilters">
                            @include('elements.icon', [
                                'icon' => 'filter-outline',
                                'variant' => 'medium',
                                'centered' => true,
                ...
                <div class="py-2 m-pt-70">
                    @if ($activeFilter == 'people')
                        <div class="mobile-search-filter collapse {{ $searchFilterExpanded ? 'show' : '' }}"
                            id="colappsableFilters">
                    ...
                    <div class="inline-border-tabs mt-3">
                        <nav class="nav nav-pills nav-justified bookmarks-nav">
                            @foreach ($availableFilters as $filter)
                                <a class="nav-item nav-link {{ $filter == $activeFilter ? 'active' : '' }}"
                                    href="{{ route('search.get', array_merge(['query' => isset($searchTerm) && $searchTerm ? $searchTerm : ''], ['filter' => $filter])) }}">
                                    <div class="d-flex justify-content-center text-bold">
                                        ...
                                        @if ($filter == 'live')
                                            <div class="blob red d-none d-md-block"></div>
                                        @endif
                                        <span
                                            class="d-none d-md-block ml-2">{{ ucfirst(trim(in_array($filter, ['videos', 'people']) ? trans_choice($filter, 2, ['number' => '']) : __(ucfirst($filter)))) }}</span>
                                    </div>
                                </a>
                            @endforeach
                        </nav>
