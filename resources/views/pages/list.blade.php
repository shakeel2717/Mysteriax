            ...
            <div class="row">
                <div class="min-vh-100 border-right col-12 pr-md-0">
                    <div class="pt-4 pl-4 px-3 d-flex justify-content-between pb-3 border-bottom">
                        <h5
                            class="mb-0 text-truncate text-bold {{ Cookie::get('app_theme') == null ? (getSetting('site.default_user_theme') == 'dark' ? '' : 'text-dark-r') : (Cookie::get('app_theme') == 'dark' ? '' : 'text-dark-r') }}">
                            {{ __($list->name) }}</h5>
                        @if ($list->isManageable)
                            <div class="mr-2">
                                <div
                                    class="dropdown {{ GenericHelper::getSiteDirection() == 'rtl' ? 'dropright' : 'dropleft' }}">
                                    <a class="btn btn-outline-primary btn-sm dropdown-toggle px-3 mb-0"
                                    ...
                                    @foreach ($list->members as $member)
                                        <div class="col-12 col-md-6 col-xl-4">
                                            @include('elements.feed.suggestion-card', [
                                                ...
                                                'isListMode' => true,
                                                'isListManageable' =>
                                                    $list->type == \App\Model\UserList::FOLLOWERS_TYPE
                                                        ? false
                                                        : true,
                                            ])
                                        </div>
