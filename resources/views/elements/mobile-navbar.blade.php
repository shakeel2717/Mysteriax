    ...
    <div class="d-flex justify-content-between w-100 py-2 px-2">
        ...
        @if (Auth::check())
            @if (GenericHelper::isUserVerified())
                <a href="{{ route('my.earning.index') }}"
                    class="h-pill h-pill-primary nav-link d-flex justify-content-between px-3 {{ Route::currentRouteName() == 'my.earning.index' ? 'active' : '' }}">
                    <div class="d-flex justify-content-center align-items-center">
            ...
            @if (!getSetting('site.hide_create_post_menu'))
                @if (GenericHelper::isEmailEnforcedAndValidated())
                    <a {!! GenericHelper::isUserVerified() ? '' : 'onclick="return false;"' !!} href="{{ route('posts.create') }}"
                        class="h-pill h-pill-primary nav-link d-flex justify-content-between px-3 {{ Route::currentRouteName() == 'posts.create' ? 'active' : '' }}">
                        <div class="d-flex justify-content-center align-items-center">
                            <div
                                class="icon-wrapper {!! GenericHelper::isUserVerified() ? '' : 'text-muted' !!} d-flex justify-content-center align-items-center">
                                @include('elements.icon', [
