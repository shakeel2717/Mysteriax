<nav
    ...
    <ul class="list-unstyled menu-elements p-0">
        @if (GenericHelper::isEmailEnforcedAndValidated())
            ...
            @endif
            <li class="{{ Route::currentRouteName() == 'my.vault.index' ? 'active' : '' }}">
                <a class="scroll-link d-flex align-items-center" href="{{ route('my.vault.index') }}">
                    @include('elements.icon', [
                        'icon' => 'image-outline',
                        'variant' => 'medium',
                        'centered' => false,
            ...
            <li>
                ...
                <ul class="collapse list-unstyled" id="otherSections">
                    @foreach (LocalesHelper::getAvailableLanguages() as $languageCode)
                        <li>
                            <a class="scroll-link d-flex align-items-center"
                                href="{{ route('language', ['locale' => $languageCode]) }}">{{ __(LocalesHelper::getLanguageName($languageCode)) }}</a>
                        </li>
                    @endforeach
                </ul>
            </li>
